...
async function main() {
    ...
    try {
        ...
        let appPage = pages.find(p => p.url().includes('50864') || p.url().includes('localhost'));
        if (!appPage) {
            console.log("Could not find app page. Picking first non-devtools page.");
            appPage = pages.find(p => !p.url().includes('devtools_frontend')) || pages[0];
        }
