...
export const Scraper = () => {
    ...
    const [stats, setStats] = useState({ totalScanned: 0, success: 0, failed: 0, excluded: 0 });
    ...
    const startTestRun = async () => {
        ...
        const newRecord: RunRecord = {
            ...
            mode,
            stats: { totalScanned: 0, success: 0, failed: 0, excluded: 0 },
            logs: [],
            failureReasons: [],
            failedUrls: []
        };
        ...
        setLogs([]);
        setStats({ totalScanned: 0, success: 0, failed: 0, excluded: 0 });
        setProgress(0);
                    ...
                    {(isRunning || logs.length > 0) && (
                        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-[#0d1117] border-t border-slate-200 dark:border-slate-800 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] p-4 md:px-8 max-h-[40vh] flex flex-col">
                            <div className="max-w-[1200px] w-full mx-auto flex flex-col h-full gap-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex gap-4 text-sm">
                                            ...
                                            <span className="font-bold text-slate-900 dark:text-white font-mono">合計: {stats.totalScanned} / {stopLimit}</span>
                                        </div>
                                    </div>
                                ...
                                {/* P8.9: Download Actions for Completed Run */}
                                {!isRunning && stats.totalScanned> 0 && (
                                    <div className="flex flex-wrap gap-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                                        ...
                                        <div className="flex gap-2">
                                            <a href={`/api/runs/${currentRunId || 'latest'}/files/amazon`} target="_blank" className="px-4 py-2 bg-yellow-500 hover:bg-yellow-400 text-white rounded text-sm font-bold shadow-sm transition-colors flex items-center gap-1">
                                                <span className="material-symbols-outlined text-sm">shopping_cart</span>
                                                Amazon TSV
                                            </a>
                                            <a href={`/api/runs/${currentRunId || 'latest'}/files/profit`} target="_blank" className="px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded text-sm font-bold shadow-sm transition-colors flex items-center gap-1">
                                                <span className="material-symbols-outlined text-sm">payments</span>
                                                利益計算シート
                                            </a>
                                            <a href={`/api/runs/${currentRunId || 'latest'}/files/raw`} target="_blank" className="px-4 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded text-sm font-bold shadow-sm transition-colors flex items-center gap-1">
